...
Bubble → n8n webhook → calls Express server endpoint → returns Top 2 JSON → n8n response node → Bubble

## Reflections

With more time, I would have introduced stronger guardrails to improve the robustness of code generated by Claude. Post-development testing revealed minor issues in several helper functions that likely would have been identified earlier if I had adopted a Test-Driven Development (TDD) approach from the outset. Writing tests first would have helped validate assumptions, catch edge cases early, and ensure more reliable outputs.

TDD would also have encouraged better separation of concerns, allowing behaviours to be more effectively isolated and individual functions to be assessed independently. This would improve both code quality and confidence in the system as it scales.

Additionally, refactoring helper functions into a dedicated utils module will significantly improve maintainability, which is why I have seperated them into seperate files. For example, if the logic for calculating an average score were to change, updating a single function (e.g. meanScore) in one location would propagate consistently across the codebase. This modular structure reduces duplication, simplifies updates, and makes the system easier to extend over time.
